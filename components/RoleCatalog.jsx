...
export default function RoleCatalog({ compact = false }) {
  ...
  return <div>
    ...
    <div className="hide-scrollbar mb-9 flex gap-2 overflow-x-auto pb-2 md:flex-wrap md:overflow-visible md:pb-0"><button onClick={()=>setActive('all')} className={`shrink-0 whitespace-nowrap rounded-full px-4 py-2 text-sm font-semibold transition ${active==='all'?'bg-slate-950 text-white':'border border-slate-200 bg-white text-slate-600 hover:border-orange-200 hover:text-orange-600'}`}>All Roles</button>{roleCategories.map(cat=><button key={cat.id} onClick={()=>setActive(cat.id)} className={`shrink-0 whitespace-nowrap rounded-full px-4 py-2 text-sm font-semibold transition ${active===cat.id?'bg-slate-950 text-white':'border border-slate-200 bg-white text-slate-600 hover:border-orange-200 hover:text-orange-600'}`}>{cat.label}</button>)}</div>
