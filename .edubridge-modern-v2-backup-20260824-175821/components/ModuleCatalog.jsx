...
export default function ModuleCatalog({ compact = false }) {
  ...
  const display = compact ? filtered.slice(0, 18) : filtered;
  ...
  return (
    <div>
      ...
      <div className="hide-scrollbar mb-9 flex gap-2 overflow-x-auto pb-2">
        <button onClick={() => setActive('all')} className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-semibold transition ${active === 'all' ? 'bg-slate-950 text-white' : 'border border-slate-200 bg-white text-slate-600 hover:border-orange-200 hover:text-orange-600'}`}>All Modules</button>
        {moduleCategories.map((cat) => <button key={cat.id} onClick={() => setActive(cat.id)} className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-semibold transition ${active === cat.id ? 'bg-slate-950 text-white' : 'border border-slate-200 bg-white text-slate-600 hover:border-orange-200 hover:text-orange-600'}`}>{cat.label}</button>)}
      </div>
